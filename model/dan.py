    ...
    def forward(self, batch_size, max_num_views, num_views, x):
        ...
        for i in range(0, batch_size):
            ...
            if not self.training:
                ...
                I = torch.eye(num_views[i])
                for j in range(len(attention_list) - 1, -1, -1):
                    I = torch.matmul(I, attention_list[j].squeeze(0).cpu())

                for j in range(0, num_views[i]):
                    I[j] = I[j] * utilization[j]
                    utilization[j] = I[:, j:(j + 1)].sum()
