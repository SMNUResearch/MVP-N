...
class MultiHeadAttention(nn.Module):
    def __init__(self, d_model, d_q, d_k, d_v, num_heads, dropout=0.1):
        ...
        nn.init.normal_(self.w_q.weight, mean=0, std=math.sqrt(2.0 / (self.d_model + self.d_q)))
        nn.init.normal_(self.w_k.weight, mean=0, std=math.sqrt(2.0 / (self.d_model + self.d_k)))
        nn.init.normal_(self.w_v.weight, mean=0, std=math.sqrt(2.0 / (self.d_model + self.d_v)))
        nn.init.xavier_normal_(self.fc.weight)

        self.layer_norm = nn.LayerNorm(self.d_model)
        self.dropout = nn.Dropout(dropout)
        self.attention = ScaledDotProductAttention(math.sqrt(self.d_k), dropout=dropout)
