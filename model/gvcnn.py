...
class GVCNN(nn.Module):
    ...
    def group_scheme(self, final_views, discrimination_scores, M, max_num_views):
        interval = 1.0 / M
        ...
        for i, group in enumerate(index_group):
            contribution = group_contribution[i] / len(group)
            for j in group:
                utilization[j] = contribution
    ...
    def forward(self, batch_size, max_num_views, num_views, x, use_utilization=False):
        ...
        for i in range(0, batch_size):
            ...
            group_views, group_weights, utilization = self.group_scheme(f, discrimination_scores, self.M, max_num_views)
        ...
        if use_utilization:
            ...
            return self.classifier(k), k, u, final_views

        return self.classifier(k), k, final_views
