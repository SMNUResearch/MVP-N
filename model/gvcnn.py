...
class GVCNN(nn.Module):
    ...
    def group_scheme(self, final_views, discrimination_scores, M, max_num_views, use_utilization):
        interval = 1.0 / M
        view_group = [[] for i in range(M)]
        score_group = [[] for i in range(M)]
        index_group = [[] for i in range(M)]
        for i in range(0, M):
            left = i * interval
            right = (i + 1) * interval
        ...
        if use_utilization:
            index_group = [index_list for index_list in index_group if len(index_list) > 0]
            group_contribution = [group_weight / sum(group_weights) for group_weight in group_weights]
            utilization = torch.zeros(max_num_views)
            for i in range(0, len(index_group)):
                for j in range(0, len(index_group[i])):
                    utilization[index_group[i][j]] = group_contribution[i] / len(index_group[i])

            return group_views, group_weights, utilization
        else:
    ...
    def forward(self, batch_size, max_num_views, num_views, x, use_utilization=False):
        ...
        for i in range(0, batch_size):
            ...
            if use_utilization:
                group_views, group_weights, utilization = self.group_scheme(f, discrimination_scores, self.M, max_num_views, use_utilization)
                u.append(utilization)
            else:
                group_views, group_weights = self.group_scheme(f, discrimination_scores, self.M, max_num_views, use_utilization)
