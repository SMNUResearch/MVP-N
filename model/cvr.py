...
class CVR(nn.Module):
    ...
    def forward(self, batch_size, max_num_views, num_views, x):
        ...
        for i in range(0, batch_size):
            ...
            z = z.unsqueeze(0)
            out0, attention_MHA1 = self.MHA1(z, z, z)
            out0 = self.ff1(out0)
            out1, attention_otk = self.otk_layer(out0)
            out1 = self.ff2(out1)
            pos0 = tool.normalize(self.coord_encoder(out1), self.norm_eps)
            pos1 = self.coord_decoder(pos0)
            out2, attention_MHA2 = self.MHA2(out1 + pos1, out1 + pos1, out1)
            out2 = self.ff3(out2)
            pooled_view = out2.mean(1)
            view_contribution = torch.zeros(max_num_views)
            w1 = torch.mean(attention_MHA1.squeeze(0), dim=0)
            w2 = attention_otk.squeeze(0)
            w3 = torch.mean(attention_MHA2.squeeze(0), dim=0)
            view_contribution[0:num_views[i]] = torch.mean(torch.matmul(w3, torch.matmul(w2, w1)), dim=0).cpu()
            k.append(pooled_view)
            u.append(view_contribution)
