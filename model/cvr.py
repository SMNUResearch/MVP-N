...
class ScaledDotProductAttention(nn.Module):
    ...
    def forward(self, q, k, v):
        ...
        attention = torch.matmul(q, k) / torch.sqrt(torch.tensor(self.d_k, dtype=q.dtype))
...
class OTKernel(nn.Module):
    def __init__(self, in_dim, out_size, norm_eps, heads, eps, max_iter):
        ...
        self.weight = Parameter(torch.Tensor(heads, out_size, in_dim))
    ...
    def forward(self, x):
        ...
        attention_weight = attention_weight.view(batch_size, self.out_size * self.heads, -1)
        output = torch.bmm(attention_weight, x)
...
class CVR(nn.Module):
    def __init__(self, model, K, feature_dim, num_heads, inner_dim, norm_eps, otk_heads, otk_eps, otk_max_iter, dropout, coord_dim):
        ...
        self.ff2 = PositionWiseFeedForwardBN(self.d_view_feature, self.d_hidden)
        ...
        self.ff3 = PositionWiseFeedForwardBN(self.d_view_feature, self.d_hidden)
    ...
    def forward(self, batch_size, max_num_views, num_views, x, use_utilization=False):
        ...
        for i in range(0, batch_size):
            ...
            z = z.unsqueeze(0)
            out0, _ = self.MHA1(z, z, z)
            out0 = self.ff1(out0)
            out1, _ = self.otk_layer(out0)
            k.append(out1)

            if use_utilization:
                utilization = torch.zeros(max_num_views)
                utilization[0:num_views[i]] = torch.ones(num_views[i]) / num_views[i]
        ...
        out2, _ = self.MHA2(out1 + pos1, out1 + pos1, out1)
        out2 = self.ff3(out2)
        k = out2.mean(1) # batch_size * num_features
        ...
        if self.training:
            return self.classifier(k), k, pos0, y
