...
from dataset.dataset_FG3D import FG3DSingleViewDataset, FG3DMultiViewDataset
...
if __name__ == '__main__':
    # set options
    opt = parser.get_parser('./config/FG3D.yaml')
    ...
    # define dataset
    train_dataset = FG3DSingleViewDataset(opt.DATA_ROOT, opt.CLASSES, 'train', use_train=True)
    ...
    # define dataset
    train_dataset = FG3DMultiViewDataset(opt.DATA_ROOT, opt.CLASSES, 'train', opt.MAX_NUM_VIEWS, use_train=True)
    valid_dataset = FG3DMultiViewDataset(opt.DATA_ROOT, opt.CLASSES, 'test', opt.MAX_NUM_VIEWS, use_train=False)
    test_dataset = FG3DMultiViewDataset(opt.DATA_ROOT, opt.CLASSES, 'test', opt.MAX_NUM_VIEWS, use_train=False)
    ...
    if opt.MV_FLAG == 'TRAIN':
        ...
        engine.test_FG3D(test_data, opt.TEST_T)
