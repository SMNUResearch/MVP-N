...
from utils.loss import LabelCrossEntropy, KDLoss, SoftBootstrapping, HardBootstrapping
from utils.loss import LabelSmoothing, BetaMixture, DynamicBootstrapping, LikelihoodRatioTest
from utils.loss import SelfAdaptiveTraining, ProgressiveLabelCorrection, OnlineLabelSmoothing
...
if __name__ == '__main__':
    ...
    # define model
    if opt.SV_TYPE == 'HS':
        model_stage1 = SVCNN(opt.NUM_CLASSES * 2, opt.ARCHITECTURE, opt.FEATURE_DIM, pretrained=True).to(device)
    else:
    ...
    valid_dataset = SingleViewDataset(opt.CLASSES, opt.GROUPS, opt.NUM_CLASSES, opt.DATA_ROOT, 'valid', opt.SV_TYPE, use_train=False)
    test_dataset = SingleViewDataset(opt.CLASSES, opt.GROUPS, opt.NUM_CLASSES, opt.DATA_ROOT, 'test', opt.SV_TYPE, use_train=False)
    ...
    valid_data = DataLoader(valid_dataset, batch_size=opt.SV_BS_TEST, num_workers=opt.NUM_WORKERS, shuffle=False, pin_memory=True, worker_init_fn=tool.seed_worker)
    test_data = DataLoader(test_dataset, batch_size=opt.SV_BS_TEST, num_workers=opt.NUM_WORKERS, shuffle=False, pin_memory=True, worker_init_fn=tool.seed_worker)
    ...
    if opt.SV_TYPE in ['CE', 'SEAL', 'HPIQ', 'HS']:
    ...
    # define engine
    engine = SingleViewEngine(model_stage1, train_data, valid_data, opt.NUM_CLASSES, opt.GROUPS, optimizer, opt.SV_TYPE, criterion, opt.SV_WEIGHT_PATH, opt.SV_OUTPUT_PATH, device, single_view=True)
    ...
    if opt.SV_FLAG == 'TRAIN':
        ...
        if opt.SV_TYPE in base_type:
            ...
            engine_student = SingleViewEngine(new_model_stage1, train_data, valid_data, opt.NUM_CLASSES, opt.GROUPS, new_optimizer, opt.SV_TYPE, criterion_student, opt.SV_WEIGHT_PATH, opt.SV_OUTPUT_PATH, device, single_view=True)
            ...
            for i in range(0, opt.SEAL_TIME + 1):
                if i == 0: # base training
                    ...
                    new_engine = SingleViewEngine(new_model_stage1, train_data, valid_data, opt.NUM_CLASSES, opt.GROUPS, new_optimizer, opt.SV_TYPE, criterion, seal_weight_path, opt.SV_OUTPUT_PATH, device, single_view=True)
                    new_engine.train_seal(opt.SV_EPOCHS, len(train_dataset), i, seal_targets=seal_targets)
        elif opt.SV_TYPE == 'OLS':
            engine.train_ols(opt.SV_EPOCHS)
        elif opt.SV_TYPE == 'HS':
            engine.train_hs(opt.SV_EPOCHS)
    elif opt.SV_FLAG == 'TEST':
        valid_dataset_mv = MultiViewDataset(opt.CLASSES, opt.NUM_CLASSES, opt.DATA_ROOT, 'valid', opt.MAX_NUM_VIEWS, use_train=False)
        test_dataset_mv = MultiViewDataset(opt.CLASSES, opt.NUM_CLASSES, opt.DATA_ROOT, 'test', opt.MAX_NUM_VIEWS, use_train=False)
        valid_data_mv = DataLoader(valid_dataset_mv, batch_size=opt.MV_BS_TEST, num_workers=opt.NUM_WORKERS, shuffle=False, pin_memory=True, worker_init_fn=tool.seed_worker)
        test_data_mv = DataLoader(test_dataset_mv, batch_size=opt.MV_BS_TEST, num_workers=opt.NUM_WORKERS, shuffle=False, pin_memory=True, worker_init_fn=tool.seed_worker)
        if opt.SV_TYPE == 'HS':
            cprint('*'*10 + ' Valid Sets ' + '*'*10, 'yellow')
            engine.test_hs(valid_data, opt.TEST_T)
            engine.score_fusion_hs(valid_data_mv, opt.TEST_T)
            cprint('*'*10 + ' Test Sets ' + '*'*10, 'yellow')
            engine.test_hs(test_data, opt.TEST_T)
            engine.score_fusion_hs(test_data_mv, opt.TEST_T)
        else:
            cprint('*'*10 + ' Valid Sets ' + '*'*10, 'yellow')
