...
class LikelihoodRatioTest(nn.Module):
    ...
    def forward(self, outputs, targets, epoch, index):
        ...
        if epoch < self.retro_epoch:
            ...
            retro_loss = sum(-A.matmul(soft_labels.reshape(-1, 1).float()).t().matmul(out) for A, soft_labels, out in zip(A_batch, soft_labels_batch, outputs_ls)) / len(index)
            loss = retro_loss + self.criterion(outputs, targets)
            ...
            with torch.no_grad():
                for i in range(self.data_length):
                    try:
                        ...
                        # <class 'RuntimeError'> Expected all tensors to be on the same device, but found at least two devices, cpu and cuda:0! (when checking argument for argument mat2 in method wrapper_mm)
                        unsolved += 1
                        continue
