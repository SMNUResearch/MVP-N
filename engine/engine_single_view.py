...
class SingleViewEngine:
    ...
    def train_base(self, epochs, data_length, save_outputs=False):
        ...
        for epoch in range(self.start_epoch, epochs):
            ...
            for _, (label, image, _, image_index) in enumerate(self.train_data):
    ...
    def train_bs(self, epochs):
        ...
        for epoch in range(self.start_epoch, epochs):
            total_loss = 0
            for _, (label, image, _, _) in enumerate(self.train_data):
                inputs = image.to(self.device)
                targets = label.to(self.device)
                self.optimizer.zero_grad()
                outputs = self.model(inputs)
    ...
    def valid(self):
        ...
        with torch.no_grad():
            for _, (label, image, _, _) in enumerate(self.valid_data):
                inputs = image.to(self.device)
                targets = label.to(self.device)
                outputs = self.model(inputs)
    ...
    def get_HPIQ(self):
        ...
        for i in range(0, self.num_classes):
            class_name = '%04d' % i
            group = []
            for _, x in enumerate(self.groups):
                if class_name in x:
                    group = x
    ...
    def test(self, data, T):
        ...
        with torch.no_grad():
            for _, (label, image, path, _) in enumerate(data):
                inputs = image.to(self.device)
                targets = label.to(self.device)
                outputs = self.model(inputs)
