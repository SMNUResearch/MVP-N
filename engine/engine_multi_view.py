...
class MultiViewEngine:
    ...
    def save_model_weights(self, overall_accuracy):
    ...
    def train_base(self, epochs):
        ...
        for epoch in range(self.start_epoch, epochs):
            total_loss = 0
            for index, (label, image, num_views, _) in enumerate(self.train_data):
                ...
                outputs, _, _ = self.model(B, V, num_views, inputs)
            ...
            # save best model
            self.save_model_weights(overall_accuracy)
    ...
    def train_cvr(self, epochs, vert, weight_factor, norm_eps):
        ...
        for epoch in range(self.start_epoch, epochs):
            ...
            for index, (label, image, num_views, _) in enumerate(self.train_data):
                ...
                B, V, C, H, W = inputs.shape
                inputs = inputs.view(-1, C, H, W)
                outputs, _, pos, _ = self.model(B, V, num_views, inputs)
                pos_loss = vector_norm(tool.normalize(vert, norm_eps) - tool.normalize(pos, norm_eps), ord=2, dim=-1).mean()
    ...
    def valid(self):
        ...
        with torch.no_grad():
            for _, (label, image, num_views, _) in enumerate(self.valid_data):
                inputs = image.to(self.device)
                targets = label.to(self.device)
                B, V, C, H, W = inputs.shape
                inputs = inputs.view(-1, C, H, W)
                ...
                correct_points = torch.sum(results.long()).cpu().data
                all_correct_points += correct_points
                all_points += results.size()[0]

        overall_accuracy = float(all_correct_points) / all_points
    ...
    def test(self, data, T):
        ...
        with torch.no_grad():
            for _, (label, image, num_views, marks) in enumerate(data):
                inputs = image.to(self.device)
                targets = label.to(self.device)
                B, V, C, H, W = inputs.shape
                inputs = inputs.view(-1, C, H, W)
                outputs, _, utilization, _ = self.model(B, V, num_views, inputs, use_utilization=True)
                prediction = torch.max(outputs, 1)[1]
                transform_targets = torch.max(targets, 1)[1]
                results = prediction == transform_targets
    ...
    def confusion_matrix(self, data):
        ...
        with torch.no_grad():
            for _, (label, image, num_views, _) in enumerate(data):
                inputs = image.to(self.device)
                targets = label.to(self.device)
                B, V, C, H, W = inputs.shape
                inputs = inputs.view(-1, C, H, W)
